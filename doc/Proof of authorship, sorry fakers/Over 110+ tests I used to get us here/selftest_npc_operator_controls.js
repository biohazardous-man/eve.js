...
function moveSessionShipToFirstGate(scene, session) {
  const gateEntity = scene.staticEntities.find((entity) => entity && entity.kind === "stargate");
  assert(gateEntity, "expected stargate anchor in operator control test scene");
  const nextPosition = {
    x: Number(gateEntity.position && gateEntity.position.x || 0) + 1_000,
    y: Number(gateEntity.position && gateEntity.position.y || 0),
    z: Number(gateEntity.position && gateEntity.position.z || 0) + 1_000,
  };
  const shipEntity = scene.getShipEntityForSession(session);
  assert(shipEntity, "expected live ship entity for operator control test session");
  shipEntity.position = { ...nextPosition };
  shipEntity.targetPoint = { ...nextPosition };
  shipEntity.velocity = { x: 0, y: 0, z: 0 };
  shipEntity.mode = "STOP";
  shipEntity.speedFraction = 0;
  session.shipItem.spaceState.position = { ...nextPosition };
...
function main() {
  ...
  try {
    ...
    assert.strictEqual(localRatEntity.nativeNpc, true);
    assert.strictEqual(localConcordEntity.nativeNpc, true);
    ...
    moveSessionShipToFirstGate(scene, session);
    ...
    assert(
      systemSummaries
        ...
        .every((summary) => summary.transient === true),
      "dynamic gate rats should now be runtime-only native controllers",
    );
    assert(
      systemSummaries
        .filter((summary) => summary.startupRuleID === gateConcordRuleID)
        .every((summary) => summary.transient === true),
      "dynamic gate CONCORD should stay runtime-only native controllers",
    );
