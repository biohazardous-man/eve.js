...
const MAX_EXPECTED_RESPONSE_WARP_DISTANCE_METERS = ONE_AU_IN_METERS * 3.25;
const MIN_EXPECTED_RESPONSE_WARP_DISTANCE_METERS = ONE_AU_IN_METERS * 1.75;
  ...
  try {
    ...
    responderEntities.forEach((entity) => {
      ...
      assert(
        ...
        "dense public-grid response origins should stay on the dedicated few-AU Crimewatch anchor instead of drifting absurdly far away",
      );
      assert(
        entity.warpState &&
          magnitude({
        ...
        "dense public-grid response origins should begin several AU away so the spawn stays genuinely off-grid",
      );
    });

    assert(
      responderEntityIDs.every((entityID) => !getVisibleDynamicEntityIDs(attackerSession).has(entityID)),
      "dense public-grid Crimewatch responders should not leak the far origin spawn to the pilot",
    );
    assert(
      responderEntityIDs.every((entityID) => !getVisibleDynamicEntityIDs(victimSession).has(entityID)),
      "dense public-grid Crimewatch responders should not leak the far origin spawn to the observer",
    );
    ...
    assert(
      ...
      "dense public-grid responders should acquire through the sessionless EntityWarpIn path once they enter the destination cluster",
    );
    assert(
      !responseNames.includes("WarpTo"),
      "dense-grid responders should not leak the origin departure WarpTo contract to the destination observers",
    );
  } finally {
    for (const session of sessions) {
