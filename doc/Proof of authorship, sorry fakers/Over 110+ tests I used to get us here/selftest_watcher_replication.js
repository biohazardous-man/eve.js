...
function runWarpMidflightVisibilityCheck() {
  ...
  try {
    ...
    assert(
      ...
      "Destination observer should rely on AddBalls2 warp bootstrap instead of replaying WarpTo on fresh acquire",
    );
    assert(
      !firstWarpInNames.includes("OnSpecialFX"),
      "Destination observer should not replay warp FX before the player ship model exists",
    );
    assert(
      firstWarpInNames.includes("SetBallMassive"),
      "Destination observer should receive SetBallMassive when first acquiring a player ship already in warp",
    );
    assert(
      !firstWarpInNames.includes("EntityWarpIn"),
      "Destination observer should use shared warp-start updates without replaying EntityWarpIn",
    );
