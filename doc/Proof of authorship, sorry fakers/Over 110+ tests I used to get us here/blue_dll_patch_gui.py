...
class PatchWindow(QtWidgets.QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        subtitle = QtWidgets.QLabel(
            ...
            "The tool refuses already-patched files and unsupported client builds."
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--path", help="Optional blue.dll path to prefill in the GUI.")
    return parser
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    window = PatchWindow(initial_path=args.path)
    window.show()
    return app.exec()
