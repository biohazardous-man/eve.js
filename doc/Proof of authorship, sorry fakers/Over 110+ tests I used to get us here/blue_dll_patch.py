...
DEFAULT_MANIFEST_PATH = REPO_ROOT / "PATCHED_FILES" / "blue-dll.patch.json"
...
class InspectionResult:
    ...
    def can_patch(self) -> bool:
        return self.state == "patchable_original"
...
def apply_patch_bytes(source_bytes: bytes, manifest: PatchManifest | None = None) -> bytes:
    ...
    if len(source_bytes) != manifest.source_size or source_hash != manifest.source_sha256:
    ...
    if manifest.overlay is not None:
        ...
        if sha256_bytes(source_overlay) != overlay.before_sha256:
...
def apply_patch(
    ...
    *,
    ...
    patched_bytes = apply_patch_bytes(source_bytes, manifest)
