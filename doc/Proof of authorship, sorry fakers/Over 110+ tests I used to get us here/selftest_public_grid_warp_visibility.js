...
function runDestinationGridWarpInCheck() {
  ...
  try {
    ...
    const scene = runtime.getSceneForSession(pilotSession);
    const pilotEntity = scene.getShipEntityForSession(pilotSession);
    const destinationObserverEntity =
      scene.getShipEntityForSession(destinationObserverSession);
    assert(pilotEntity);
    assert(destinationObserverEntity);
    pilotEntity.direction = { x: 1, y: 0, z: 0 };
    ...
    assert(
      ...
      "Destination observer fresh acquisition should rely on AddBalls2 warp bootstrap instead of replaying WarpTo",
    );
    assert(
      !firstWarpInNames.includes("OnSpecialFX"),
      "Destination observer fresh acquisition should not replay warp FX before the ship model exists",
    );
    assert(
      firstWarpInNames.includes("SetBallMassive"),
      "Destination observer should receive SetBallMassive on fresh acquisition",
    );
    assert(
      !firstWarpInNames.includes("EntityWarpIn"),
      "Destination observer should use shared warp-start updates instead of replaying EntityWarpIn",
    );
    assert.strictEqual(
      pilotEntity.mode,
      "WARP",
    ...
    assert(
      scene.getPublicGridClusterKeyForPosition(pilotEntity.position) ===
        scene.getPublicGridClusterKeyForEntity(destinationObserverEntity),
      "Destination observer should first acquire the player when the live warp ball enters the destination public grid cluster",
    );

    let postAcquireNames = [];
    let completionAt = null;
    let completionNames = [];
    destinationObserverSession.notifications.length = 0;
    for (
      let tickNow = firstWarpInAt + 250;
      tickNow <= completionNow + 1000;
      tickNow += 250
    ) {
      scene.tick(tickNow);
      const names = flattenDestinyPayloadNames(destinationObserverSession.notifications);
      postAcquireNames.push(...names);
      if (names.includes("Stop")) {
        completionAt = tickNow;
        completionNames = names;
        break;
      }
      destinationObserverSession.notifications.length = 0;
    }
    ...
    assert(
      !postAcquireNames.includes("WarpTo"),
      "Destination observer should not receive a separate WarpTo replay after fresh AddBalls2 bootstrap",
    );
    assert(
      !postAcquireNames.includes("EntityWarpIn"),
      "Destination observer should not receive a fresh EntityWarpIn after AddBalls2 bootstrap",
    );
...
function runCrossGridWarpDropOffCheck() {
  ...
  try {
    ...
    assert(
      !destinationAcquireNames.includes("WarpTo"),
      "Cross-grid destination acquisition should stay on the AddBalls2 warp bootstrap for player ships",
    );
    assert(
      !destinationAcquireNames.includes("OnSpecialFX"),
      "Cross-grid destination acquisition should not replay warp FX before the player ship is loaded",
    );
    assert(
      !destinationAcquireNames.includes("EntityWarpIn"),
      "Cross-grid destination acquisition should not replay EntityWarpIn for player ships",
    );
...
function runDestinationEdgeWarpPreloadCheck() {
  runtime._testing.clearScenes();
  const gridBase = buildIsolatedPosition(
    0,
    testing.PUBLIC_GRID_BOX_METERS * 4,
    0,
  );
  const pilotSession = createFakeSession(
    995131,
  ...
  const destinationObserverSession = createFakeSession(
    995132,
    996132,
    add(gridBase, { x: testing.PUBLIC_GRID_BOX_METERS * 2.2, y: 1000, z: 0 }),
    { x: -1, y: 0, z: 0 },
  );
  ...
  try {
    ...
    assert(destinationObserverEntity);
    pilotEntity.direction = { x: 1, y: 0, z: 0 };
    pilotEntity.targetPoint = add(gridBase, {
      x: testing.PUBLIC_GRID_BOX_METERS * 2.18,
      y: 1000,
      z: 0,
    });
    pilotEntity.speedFraction = 1;
    ...
    const warpResult = runtime.warpToPoint(
      pilotSession,
      pilotEntity.targetPoint,
      { targetEntityID: 40009531, stopDistance: 0, warpSpeedAU: 3 },
    );
    assert.strictEqual(warpResult.success, true);
    assert(pilotEntity.pendingWarp);
    pilotEntity.pendingWarp.requestedAtMs = baseNow - 30000;
    ...
    assert(
      acquireAt !== null,
      "Destination-edge observer should acquire the player before landing",
    );
    assert.strictEqual(
      pilotEntity.mode,
      "WARP",
      "Destination-edge observer should first acquire the ship while it is still in warp",
    );
    assert(
      scene.getPublicGridClusterKeyForPosition(pilotEntity.position) ===
        scene.getPublicGridClusterKeyForEntity(destinationObserverEntity),
      "Destination-edge observer should first acquire the ship when the live warp ball enters the destination public grid cluster",
    );
    assert(
      !acquireNames.includes("WarpTo"),
      "Destination-edge fresh acquisition should stay on AddBalls2 bootstrap first",
    );
    ...
    assert(
      !postAcquireNames.includes("WarpTo"),
      "Destination-edge observer should not receive a delayed player warp replay before landing",
    );
    if (completionAt !== null) {
      assert(completionNames.includes("SetBallPosition"));
      assert(completionNames.includes("Stop"));
...
function main() {
  ...
  const destinationEdgeWarpPreload = runDestinationEdgeWarpPreloadCheck();
