...
test("dogma bootstrap keeps loaded laser charges on the charge-state sublocation path", () => {
  const candidate = findSpaceLaserCandidate();
  const session = buildSession(candidate);
  const dogma = new DogmaService();
...
test("charge quantity transitions replay tuple-backed sublocation rows for loaded ammo", () => {
  const candidate = findSpaceLaserCandidate();
  const session = buildSession(candidate);
  const dogma = new DogmaService();
  ...
  assert.equal(
    chargeRows.some(
      (fields) =>
        ...
        Number(fields.itemID[2]) === Number(candidate.chargeItem.typeID) &&
        Number(fields.locationID) === Number(candidate.ship.itemID),
    ),
    true,
    "Expected charge quantity transitions to emit a tuple-backed slot charge row for the HUD",
  );
  assert.equal(
...
test("in-space ammo type swaps load the new tuple dogma item and finish on a clean HUD tuple row", async () => {
  ...
  assert.equal(
    ...
    true,
    "Expected live ammo swaps to create the new tuple through an attributeQuantity change so tooltip dogma can load it",
  );
  assert.equal(
...
test("in-space inventory sync suppresses real fitted charge rows and keeps slot ammo tuple-backed", () => {
  const candidate = findSpaceLaserCandidate();
  const session = buildSession(candidate);
  const dogma = new DogmaService();
  ...
  assert.equal(
    ...
    false,
    "Expected in-space inventory sync to suppress real fitted charge inventory rows",
  );
  assert.equal(
    extractOnItemChangeRows(session.notifications).length,
    0,
    "Expected raw in-space inventory sync to drop the fitted charge row entirely and leave tuple-backed ammo state to the charge transition path",
  );
});
