...
class UserService extends BaseService {
  ...
  Handle_UserLogOffCharacter(args, session) {
    ...
    const selectedCharacterId = session
      ...
      : 0;
    log.info(`[userSvc] UserLogOffCharacter called (charID=${charId || 0})`);
    ...
    for (const offlineCharId of offlineCandidates) {
      ...
      if (!offlineResult.success) {
        log.warn(
          `[userSvc] Failed to mark character ${offlineCharId} offline: ${offlineResult.errorMsg}`,
        );
        continue;
      }
      log.debug(
        `[userSvc] Marked character ${offlineCharId} offline (changed=${offlineResult.changed !== false})`,
      );
    }
    ...
    if (removedXmppClients > 0) {
      log.debug(
        `[userSvc] Removed ${removedXmppClients} XMPP room client(s) for logged-off character`,
      );
    }
