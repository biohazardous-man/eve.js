...
function itemAppliesPassiveStats(item) {
  ...
  if (!typeHasEffectName(item.typeID, "online")) {
    return true;
  }
...
function collectPassiveShipAttributeModifiers(fittedItems = []) {
  ...
  const addModifier = (attributeID, operation, value, item) => {
    const numericAttributeID = toInt(attributeID, 0);
    const numericOperation = toInt(operation, -1);
    const numericValue = toFiniteNumber(value, NaN);
    if (
    ...
    const key = `${numericAttributeID}:${numericOperation}`;
    ...
    modifiers.get(key).entries.push({
      value: numericValue,
      item,
    });
  };
  ...
  for (const item of Array.isArray(fittedItems) ? fittedItems : []) {
    ...
    const effectIDs = Array.isArray(getTypeDogmaRecord(item.typeID)?.effects)
      ? getTypeDogmaRecord(item.typeID).effects
      : [];
    const typeAttributes = getTypeDogmaAttributes(item.typeID);
    for (const effectID of effectIDs) {
      ...
      for (const modifierInfo of effect.modifierInfo) {
        ...
        const modifyingAttributeID = toInt(modifierInfo.modifyingAttributeID, 0);
        const rawValue =
          modifyingAttributeID > 0
            ? typeAttributes[String(modifyingAttributeID)]
            : undefined;
...
function getStackedMultiplierFactor(entries = []) {
  ...
  const factors = entries
    .map((entry) => toFiniteNumber(entry && entry.factor, NaN))
...
function applyPassiveShipAttributeModifiers(attributes, modifiers) {
  for (const modifierGroup of modifiers.values()) {
    const attributeID = modifierGroup.attributeID;
    const currentValue = toFiniteNumber(attributes[attributeID], NaN);
    if (!Number.isFinite(currentValue)) {
      continue;
    }

    switch (modifierGroup.operation) {
      case DOGMA_OP_PRE_MUL:
      case DOGMA_OP_POST_MUL: {
        const factor = getStackedMultiplierFactor(
          modifierGroup.entries.map((entry) => ({
            factor: toFiniteNumber(entry.value, 1),
          })),
        );
        attributes[attributeID] = round6(currentValue * factor);
        break;
      }
      case DOGMA_OP_MOD_ADD: {
        const totalAdd = modifierGroup.entries.reduce(
          (sum, entry) => sum + toFiniteNumber(entry.value, 0),
          0,
        );
        attributes[attributeID] = round6(currentValue + totalAdd);
        break;
      }
      case DOGMA_OP_MOD_SUB: {
        const totalSub = modifierGroup.entries.reduce(
          (sum, entry) => sum + toFiniteNumber(entry.value, 0),
          0,
        );
        attributes[attributeID] = round6(currentValue - totalSub);
        break;
      }
      case DOGMA_OP_POST_PERCENT: {
        const factor = getStackedMultiplierFactor(
      ...
      case DOGMA_OP_POST_ASSIGNMENT: {
        const lastEntry =
          modifierGroup.entries[modifierGroup.entries.length - 1] || null;
        if (lastEntry) {
...
function buildShipResourceState(charID, shipItem, options = {}) {
  ...
  return {
    ...
    cpuOutput: round6(toFiniteNumber(derivedAttributes[ATTRIBUTE_CPU_OUTPUT], 0)),
    powerOutput: round6(toFiniteNumber(derivedAttributes[ATTRIBUTE_POWER_OUTPUT], 0)),
    cpuLoad: round6(cpuLoad),
    powerLoad: round6(powerLoad),
    cargoCapacity: round6(toFiniteNumber(derivedAttributes[ATTRIBUTE_CAPACITY], 0)),
    maxVelocity: round6(toFiniteNumber(derivedAttributes[ATTRIBUTE_MAX_VELOCITY], 0)),
    agility: round6(toFiniteNumber(derivedAttributes[ATTRIBUTE_AGILITY], 0)),
    mass: round6(toFiniteNumber(derivedAttributes[ATTRIBUTE_MASS], 0)),
    maxTargetRange: round6(
      toFiniteNumber(derivedAttributes[ATTRIBUTE_MAX_TARGET_RANGE], 0),
    ),
    maxLockedTargets: round6(
      toFiniteNumber(derivedAttributes[ATTRIBUTE_MAX_LOCKED_TARGETS], 0),
    ),
    signatureRadius: round6(
      toFiniteNumber(derivedAttributes[ATTRIBUTE_SIGNATURE_RADIUS], 0),
    ),
    cloakingTargetingDelay: round6(
      toFiniteNumber(derivedAttributes[ATTRIBUTE_CLOAKING_TARGETING_DELAY], 0),
    ),
    scanResolution: round6(
      toFiniteNumber(derivedAttributes[ATTRIBUTE_SCAN_RESOLUTION], 0),
    ),
    capacitorCapacity: round6(
      toFiniteNumber(derivedAttributes[ATTRIBUTE_CAPACITOR_CAPACITY], 0),
    ),
    capacitorRechargeRate: round6(
      toFiniteNumber(derivedAttributes[ATTRIBUTE_CAPACITOR_RECHARGE_RATE], 0),
    ),
    shieldCapacity: round6(
      toFiniteNumber(derivedAttributes[ATTRIBUTE_SHIELD_CAPACITY], 0),
    ),
    shieldRechargeRate: round6(
      toFiniteNumber(derivedAttributes[ATTRIBUTE_SHIELD_RECHARGE_RATE], 0),
    ),
    armorHP: round6(toFiniteNumber(derivedAttributes[ATTRIBUTE_ARMOR_HP], 0)),
    structureHP: round6(
      toFiniteNumber(derivedAttributes[ATTRIBUTE_STRUCTURE_HP], 0),
    ),
    upgradeCapacity: round6(
      toFiniteNumber(derivedAttributes[ATTRIBUTE_UPGRADE_CAPACITY], 0),
    ),
