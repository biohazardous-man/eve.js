...
function normalizeControlState(state) {
  ...
  const rawSystemGateControls =
    source.systemGateControls && typeof source.systemGateControls === "object"
      ? source.systemGateControls
      : {};
  for (const [systemID, rawControl] of Object.entries(rawSystemGateControls)) {
    const normalizedSystemID = toPositiveInt(systemID, 0);
    if (!normalizedSystemID) {
      continue;
    }
    const normalizedControl = normalizeSystemGateControl(
      normalizedSystemID,
      rawControl,
    );
    if (
      normalizedControl.gateConcordEnabled === true ||
      normalizedControl.gateRatEnabled === true
    ) {
      systemGateControls[String(normalizedSystemID)] = normalizedControl;
    }
  }
...
function getSystemGateControl(systemID) {
  ...
  return normalizeSystemGateControl(
    normalizedSystemID,
    state.systemGateControls[String(normalizedSystemID)] || null,
  );
}
...
function setSystemGateControl(systemID, updates = {}) {
  ...
  const merged = normalizeSystemGateControl(
    normalizedSystemID,
    {
      ...state.systemGateControls[key],
      ...updates,
    },
  );

  if (!merged.gateConcordEnabled && !merged.gateRatEnabled) {
    delete state.systemGateControls[key];
  } else {
    state.systemGateControls[key] = merged;
  }
